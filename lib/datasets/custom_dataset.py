...
class custom_dataset(Dataset):
    """
    Custom Dataset Class - Handles data loading and augmentation for imu human custom recorded data
    Able to handle heterogeneous datasets (different annotations available for different datasets).
    You need to update the path to each dataset in utils/config.py.
    """

    def __init__(self, img_paths, boxes, camera_data, ignore_3d=False, use_augmentation=True, is_train=True,
                normalization=False, cropped=False, crop_size=224):
        super(custom_dataset, self).__init__()
        
        dataset = 'imu_human_recordings'
        ...
        # self.img_dir = img_dir #config.DATASET_FOLDERS[dataset]

        self.imgname = img_paths#np.array([os.path.join(img_dir, subdir) for subdir in sorted(os.listdir(img_dir))], dtype=np.string_)#self.data['imgname'].astype(np.string_)
        ...
        try:
            ...
            self.J_regressor = torch.from_numpy(np.load(os.path.join('ReFit', JOINT_REGRESSOR_H36M))).float()
        ...
        for box in boxes:
            center, scale = boxes_2_cs(box)
            self.center.append(center.flatten())
            self.scale.append(scale.flatten())
        self.center = np.array(self.center)
        self.scale = np.array(self.scale).flatten()
        self.sc = 1.0
    ...
    def __getitem__(self, index):
        ...
        imgpath = self.imgname[index]
        try:
            img = cv2.imread(imgpath)[:,:,::-1].copy().astype(np.float32)
        except TypeError:
            logger.info(f"cv2 loading error image={imgpath}")
